...
export default class FolderWatcher {
	...
	public onAdd: ((filePath: string, type: ItemType) => void) | null;

	public onChange: ((filePath: string) => void) | null;

	public onRemove: ((filePath: string) => void) | null;

	public watchers: Map<string, fs.FSWatcher>; // path -> watcher

	public constructor(folder: string, recursive = true) {
