...
export class CommandValidator {
	public static isAllowedCommand(
		...
		allowed: boolean;
		cooldown?: { timeLeft: number; type: string };
		reason: string | keyof typeof config.deniedCommandReplies.specific | null;
	} {
	...
	private static checkOptionalConditions(
		...
		const optionalConditions: boolean[] = [
			...
			memberRoles
				? command.optionalAllowedRoles?.some((roleId: string) => memberRoles.cache.has(roleId))
				: undefined,
	...
	private static checkCooldown(
		...
		user: User,
		guild: Guild | null,
	): { onCooldown: boolean; timeLeft?: number; type?: "global" | "guild" | "user" } {
		...
		const applyCooldown = (key: string, cooldown: number, storage: Collection<string, string>): number | null => {
			...
			if (expiration && now < Number.parseInt(expiration, 10)) {
				return Number.parseInt(expiration, 10) - now;
			}
